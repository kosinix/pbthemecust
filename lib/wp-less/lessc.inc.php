...
if(!class_exists("lessc_parser")){
    class lessc_parser {
    	...
    	// these properties will supress pbtheme unless it's inside parenthases
    	static protected $supressPBThemeProps =
    	...
    	protected function expression(&$out) {
    		if ($this->value($lhs)) {
    			...
    			// look for / shorthand
    			if (!empty($this->env->supressedPBTheme)) {
    				unset($this->env->supressedPBTheme);
    				$s = $this->seek();
    	...
    	protected function expHelper($lhs, $minP) {
    		...
    		while (true) {
    			...
    			if ($this->match(self::$operatorString.($needWhite ? '\s' : ''), $m) && self::$precedence[$m[1]] >= $minP) {
    				if (!$this->inParens && isset($this->env->currentProperty) && $m[1] == "/" && empty($this->env->supressedPBTheme)) {
    					foreach (self::$supressPBThemeProps as $pattern) {
    						if (preg_match($pattern, $this->env->currentProperty)) {
    							$this->env->supressedPBTheme = true;
    							break 2;
    						}
    					}
    				}
